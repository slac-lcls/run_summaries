...
def statusStats(det_name, run, printme=False, request_run=None):
    ...
    shortid = uc.detector_name_short(detrawid)
    runnum = run.runnum
    #as far as I can see, we have pedestals, rms, pixel_status and pixel_gain
    if request_run:
        try:
            status = calib_constants(shortid, exp=expname, ctype="pixel_status", run=request_run)[0]
        except:
            return {}
    else:
...
def allPlots(
    ...
    try:
        if len(prev_darks)>0:
            ...
            peds_pre = calib_constants(shortid, exp=expname, ctype="pedestals", run=prev_dark)[0]
